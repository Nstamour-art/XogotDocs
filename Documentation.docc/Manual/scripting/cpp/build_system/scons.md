...
It is modeled after <doc:index>, and some commands available there are also
...
For more information, please check out the <doc:index>.
...
Godot loads GDExtensions by finding <doc:gdextension_file> files in the project directory.
...
<doc:gdextension_cpp_example>, provide example `.gdextension` files for GDExtensions
...
- If you <doc:index> with different options than the official builds (e.g. `disable_3d=yes` or `precision=double`).
